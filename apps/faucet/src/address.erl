...
-export([addr2chain/2, check/1, encodekey/1, paddr/1,
	 parsekey/1, pub2addr/2, pub2addrraw/2, pub2caddr/2]).

paddr(PKey) ->
    Crc = erlang:crc32(PKey),
    <<"tp",
      (base58:encode(<<Crc:32/integer,
		       PKey/binary>>))/binary>>.

pub2addr(node, Pub) ->
    Hash = crypto:hash(ripemd160, Pub),
    Crc = erlang:crc32(Hash),
    base58:encode(<<76, 200, 56, 214, Crc:32/integer,
		    Hash/binary>>);
pub2addr(Ver, Pub) when is_integer(Ver) ->
    H2H3 = pub2addrraw(Ver, Pub), base58:encode(H2H3);
pub2addr({ver, Ver}, Pub) when is_integer(Ver) ->
    H2H3 = pub2addrraw(Ver, Pub), base58:encode(H2H3);
pub2addr({chain, Chain}, Pub) when is_integer(Chain) ->
    H2H3 = pub2caddrraw(Chain, Pub), base58:encode(H2H3).

pub2caddr(Chain, Pub) ->
    H2H3 = pub2caddrraw(Chain, Pub), base58:encode(H2H3).

pub2addrraw(Ver, Pub) ->
    H1 = crypto:hash(ripemd160, crypto:hash(sha256, Pub)),
    H2 = <<Ver:8/integer, H1/binary>>,
    <<H3:4/binary, _/binary>> = crypto:hash(sha256,
					    crypto:hash(sha256, H2)),
    <<H2/binary, H3/binary>>.

pub2caddrraw(Chain, Pub) ->
    H1 = crypto:hash(ripemd160, crypto:hash(sha256, Pub)),
    H2 = <<133, H1/binary, Chain:32/big>>,
    ...
    case base58:decode(Address) of
	  ...
	  <<H3:4/binary, _/binary>> = crypto:hash(sha256,
						  crypto:hash(sha256,
								...
								RipeMD:20/binary>>)),
	  Check = H3,
	  H2 = <<133, RipeMD/binary, Chain:32/big>>,
	  <<H3n:4/binary, _/binary>> = crypto:hash(sha256, H2),
	  <<H2/binary, H3n/binary>>;
      <<133, RipeMD:20/binary, OldChain:32/big,
	Check:4/binary>> ->
	  <<H3:4/binary, _/binary>> = crypto:hash(sha256,
						  <<133, RipeMD:20/binary,
						    OldChain:32/big>>),
	  Check = H3,
	  H2 = <<133, RipeMD/binary, Chain:32/big>>,
	  <<H3n:4/binary, _/binary>> = crypto:hash(sha256, H2),
...
check(Address) ->
    try case base58:decode(Address) of
	  <<Ver:8/integer, RipeMD:20/binary, Check:4/binary>> ->
	      <<H3:4/binary, _/binary>> = crypto:hash(sha256,
						      crypto:hash(sha256,
								  <<Ver:8/integer,
...
parsekey(<<"0x", BKey/binary>>) -> hex:parse(BKey);
parsekey(Base58) ->
    ...
    case B58Decode of
	  ...
	  <<H3:4/binary, _/binary>> = crypto:hash(sha256,
						  crypto:hash(sha256,
							      <<128:8/integer,
								KeyBody/binary>>)),
	  if KC == H3 andalso KS == 32 -> KeyBody;
	     KC == H3 andalso KS == 33 ->
		 <<KB:32/binary, _:1/binary>> = KeyBody, KB;
	     true -> error
	  end;
      _ -> error
    end.
